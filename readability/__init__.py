...
def single_sentence():
    ...
    check50.run("python3 readability.py").stdin("In my younger and more vulnerable years my father gave me some advice that I've been turning over in my mind ever since.").stdout("Grade 7\n").exit(0)


@check50.check(exists)
def single_sentence_other_punctuation():
    """handles punctuation within a single sentence"""
    check50.run("python3 readability.py").stdin("There are more things in Heaven and Earth, Horatio, than are dreamt of in your philosophy.").stdout("Grade 9\n").exit(0)
...
def sentence_punctuation():
    ...
    check50.run("python3 readability.py").stdin("Congratulations! Today is your day. You're off to Great Places! You're off and away!").stdout("Grade 3\n").exit(0)


@check50.check(exists)
def sentence_punctuation():
    """handles questions in passage"""
    check50.run("python3 readability.py").stdin("Would you like them here or there? I would not like them here or there. I would not like them anywhere.").stdout("Grade 2\n").exit(0)


@check50.check(exists)
def before1():
    """handles reading level before Grade 1"""
    check50.run("python3 readability.py").stdin("One fish. Two fish. Red fish. Blue fish.").stdout("Before Grade 1\n").exit(0)


@check50.check(exists)
def grade16plus():
    """handles reading level at Grade 16+"""
    check50.run("python3 readability.py").stdin("A large class of computational problems involve the determination of properties of graphs, digraphs, integers, arrays of integers, finite families of finite sets, boolean formulas and elements of other countable domains.").stdout("Grade 16\+\n", "Grade 16+\n").exit(0)
